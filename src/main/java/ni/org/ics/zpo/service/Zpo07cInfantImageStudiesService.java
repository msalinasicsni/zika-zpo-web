...
public class Zpo07cInfantImageStudiesService {
    ...
    public List<Zpo07cInfantImageStudies> getZpo07cInfantImageStudiesByUser(String username){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("FROM Zpo07cInfantImageStudies zpo07 where zpo07.pasive = '0' and zpo07.recordId in (select zpo00.recordId from Zpo00Screening zpo00 where zpo00.pasive = '0' and zpo00.preScreenId in (select recId from ZpoPreScreening zpPre where zpPre.pasive = '0' and zpPre.cs in " +
