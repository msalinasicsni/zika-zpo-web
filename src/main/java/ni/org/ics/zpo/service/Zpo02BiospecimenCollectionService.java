...
public class Zpo02BiospecimenCollectionService {
    ...
    public List<Zpo02BiospecimenCollection> getZpo02BiospecimenCollectionByUser(String username){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("FROM Zpo02BiospecimenCollection zpo02 where zpo02.pasive = '0' and zpo02.recordId in (select zpo00.recordId from Zpo00Screening zpo00 where zpo00.pasive = '0' and zpo00.preScreenId in (select recId from ZpoPreScreening zpPre where zpPre.pasive = '0' and zpPre.cs in " +
