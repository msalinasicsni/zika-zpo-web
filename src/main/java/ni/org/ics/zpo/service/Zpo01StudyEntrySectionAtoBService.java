...
public class Zpo01StudyEntrySectionAtoBService {
    ...
    public List<Zpo01StudyEntrySectionAtoB> getZpo01StudyEntrySectionAtoBByUser(String username){
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("FROM Zpo01StudyEntrySectionAtoB zpo01 where zpo01.pasive = '0' and zpo01.recordId in (select zpo00.recordId from Zpo00Screening zpo00 where zpo00.pasive = '0' and zpo00.preScreenId in (select recId from ZpoPreScreening zpPre where zpPre.pasive = '0' and zpPre.cs in " +
